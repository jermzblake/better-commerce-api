...
import { BetterError } from '../lib/errors'

// Lives at the very beginning of the app chain, catches any errors that occur in the app
...
export const errorEmitter = async (ctx: Context, next: Next) => {
    try {
        ...
        const error: BetterError = !(e instanceof BetterError)
        // @ts-ignore
            ? new BetterError(__filename, e.name || e.message || e, e)
            : e
...
export const errorHandler = async (err: BetterError, ctx: Context) => {
